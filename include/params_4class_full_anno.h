...
class Params
{
  public:
    ...
    const char *class_name [num_classes] = { "Car","Cyclist","Truck","Pedestrian",};
    const float min_x_range = -69.12;
    const float max_x_range = 69.12;
    const float min_y_range = -79.36;
    const float max_y_range = 79.36;
    const float min_z_range = -3.0;
    const float max_z_range = 1.0;
    // the size of a pillar
    const float pillar_x_size = 0.32;
    const float pillar_y_size = 0.32;
    ...
    const float anchor_bottom_heights[num_classes] = {-1.78,-0.6,-1.78,-0.6,};
