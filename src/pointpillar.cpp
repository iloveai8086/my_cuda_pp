...
*/
    ...
    for (int i = 0; i < num_obj; i++) {
        ...
        auto Bb = Bndbox(output[i * 9],
                         ...
                         output[i * 9 + 8]);
        //std::cout<<output[i * 9] <<' '<<output[i * 9 + 1]<<' '<<output[i * 9 + 2]<<' '<<output[i * 9 + 3]<<' '<<output[i * 9 + 4]<<' '<<output[i * 9 + 5]<<' '<<output[i * 9 + 6]<<' '<<static_cast<int>(output[i * 9 + 7])<<' '<<output[i * 9 + 8]<<std::endl;
        res_.push_back(Bb);
    }

//  std::cout<<typeid(res_)<<std::endl;
    nms_cpu(res_, params_.nms_thresh, nms_pred);
//  for (auto i = res_.begin();i!=res_.end();++i){
//	std::cout<<i<<std::endl;
//	}
