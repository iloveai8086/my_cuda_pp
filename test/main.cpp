...
// std::string Model_File = "/home/ros/CLionProjects/CUDA-PointPillars2/model/pointpillar.onnx_best4";  // 之前FP32推理的模型
std::string Model_File = "/home/ros/CLionProjects/CUDA-PointPillars2/model/pointpillar_best4_3.onnx"; // 新生成的fp16的engine对应的onnx
...
//    dataFile += "00000.bin";
        ...
        buffer.reset((char *) data);  // 函数的作用是将引用计数减1，停止对指针的共享，除非引用计数为0，否则不会发生删除操作，里面是强转？
        ...
        unsigned int points_data_size = points_size * 4 * sizeof(float);  // 就是length，占多少内存，字节
        checkCudaErrors(
                cudaMallocManaged((void **) &points_data, points_data_size)); // 先做个内存的分配 把之前定义的空指针先分配了内存，和malloc普通的有啥区别
        checkCudaErrors(
                cudaMemcpy(points_data, points, points_data_size, cudaMemcpyDefault)); // 这边就是把内存直接拷贝过去，内存地址，大小，方式
        ...
        for (auto i = nms_pred.begin(); i != nms_pred.end(); i++) {
            std::cout << i->x << ' ' << i->y << ' ' << i->z << ' ' << i->l << ' ' << i->w << ' ' << i->h << ' '
                      << i->score
                      << ' ' << i->rt << ' ' << i->id << std::endl;
        }
